...
def CompileError_NO_RET(number, internalError=None, line=0):
    ...
    io.Out.Error(
        io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", number
    )
...
def CompileBadVariable_NO_RET(variable, problem, line=0):
    io.Out.Error(
        io.TS.CMP_VAR_NOT_BOUND,
        "file::: Syntax Error, Problem with variable {0} ({1})",
        variable,
        problem,
    )
    raise program.CompileError
            ...
            if varValue.indexConstant is not None:
                ...
                if varValue.indexVariable in compileState.globalVar:
                    ...
                    return (
                        ...
                        varName,
                        varLayout[varName],
                        ("L", varValue.indexVariable, offset),
                    )
...
def CheckSpecialUAdd(programIR, functionName, line, compileState):
    ...
    (tType, tName, tOffset, info) = GetVariableInfo(
        functionName, line.target, compileState
    )
        ...
        if vInfo[tName][0] == "T":
            ...
            if line.operand.IsStrConst() and (len(line.operand.strConst) == 1):
                compileState.AddStatement(
                    "movb ${} %_cpu:acc".format(ord(line.operand.strConst[0]))
                )
                ...
                CompileError_NO_RET(
                    31, "1 character string constant or tune string element needed here"
                )
    return False
    ...
    if line.operation == "UAdd":
        ...
        if tType == "G":
            if line.operand.IsIntConst():
                compileState.AddStatement(
                    "movw ${} @{}".format(line.operand.constant, tName)
                )
                return
            else:
                if line.operand.IsSimpleVar():
                    (oType, oName, oOffset, info) = GetVariableInfo(
                        functionName, line.operand, compileState
                    )
                    if oType == "G":
...
def HandleSpecialCall(programIR, caller, callee, line, compileState):
    ...
    if callee == "Ed.CreateObject":
        ...
        if not line.args[2].IsIntConst():
            CompileError_NO_RET(
                8, "Can only use constants when setting/clearing module reg bit"
            )
        ...
        if not line.args[2].IsIntConst():
            CompileError_NO_RET(
                10, "Can only use constants when setting/clearing module reg bit"
            )
        bit = line.args[2].constant
        if bit < 0 or bit > 7:
...
def AddSimpleDriveInlineFunction(callee, compileState, line):
    ...
    if callee == "Ed.SimpleDriveStop":
        compileState.AddStatement(
            "# Inline version of {} on line:{}".format(callee, line)
        )
        ...
        compileState.AddStatement(
            "movb ${} {}".format(motorBackward, rightMotorControl)
        )
        return True
...
def CreateModRegHex(mod, reg):
    if (not mod.IsIntConst()) or (not reg.IsIntConst()):
        CompileError_NO_RET(
            12, "Can only use constants when reading/writing to module regs"
        )
...
def LoadWordVariableIntoAcc(value, functionName, compileState, stackOffset=0):
    ...
    if vType == "L":
        ...
        compileState.AddStatement(
            "bitset ${} %_index:action".format(CONTROL_INDEX_READ_16BIT)
        )
...
def StoreAccIntoWordVariable(variable, functionName, compileState, stackOffset=0):
    (vType, vName, vOffset, info) = GetVariableInfo(
        functionName, variable, compileState
    )
    if vType == "L":
        ...
        compileState.AddStatement("movb %_cpu:acc %_index:16b1cursor")
        compileState.AddStatement(
            "bitset ${} %_index:action".format(CONTROL_INDEX_WRITE_16BIT)
        )
    elif vType == "GV":
...
def LoadByteVariableIntoAcc(value, functionName, compileState):
    if value.IsStrConst():
        ...
        (vType, vName, vOffset, info) = GetVariableInfo(
            functionName, value, compileState
        )
        if vType == "GC":
            # a slice with constant index - add the offset to the value of the name
            # (the name contains the address of the start of the slice)
            ...
            compileState.AddStatement(
                "bitset ${} %_index:action".format(CONTROL_INDEX_READ_8BIT)
            )
...
def StoreAccIntoByteVariable(variable, functionName, compileState):
    ...
    if vType == "L":
        ...
        compileState.AddStatement("movb %_cpu:acc %_index:8b1cursor")
        compileState.AddStatement(
            "bitset ${} %_index:action".format(CONTROL_INDEX_WRITE_8BIT)
        )
    elif vType == "GV":
        # store the acc in the window
        compileState.AddStatement("movb %_cpu:acc %_index:8b1window")
    ...
    while index < len(line.args):
        ...
        if vFrom.IsIntConst():
            ...
            if len(vFrom.strConst) != 1:
                CompileError_NO_RET(
                    19, "String constant should have length of 1" + str(vFrom)
                )
            else:
                compileState.AddStatement("movw ${} %_cpu:acc".format(vFrom.constant))
                compileState.AddStatement("stwaw ${}".format(offset))
            ...
            CompileError_NO_RET(
                20, "Should not be a str/list constant here" + str(vFrom)
            )
        else:
            # a variable of some
...
def CompileReturn(programIR, functionName, line, compileState):
    ...
    if not line.IsVoidReturn():
        if line.returnValue.IsIntConst():
            compileState.AddStatement(
                "movw ${} %_cpu:acc".format(line.returnValue.constant)
            )
        else:
...
def CompileControlMarker(programIR, functionName, line, compileState):
    ...
    if line.name in ("While", "For") and line.end == "end":
        compileState.AddStatement(
            "bra {}".format(MakeControlLabel(line.num, line.name, "start"))
        )

    if line.end == "else":
        # route code around the else
        compileState.AddStatement(
            "bra {}".format(MakeControlLabel(line.num, line.name, "end"))
        )
...
def CompileForControl(programIR, functionName, line, compileState):
    ...
    if line.IsArray():
        ...
        compileState.AddStatement(
            "brle {}".format(MakeControlLabel(line.num, "For", "end"))
        )
        ...
        if line.constantLimit.IsIntConst():
            compileState.AddStatement(
                "movw ${} %_cpu:acc".format(line.constantLimit.constant)
            )
        else:
            LoadWordVariableIntoAcc(line.constantLimit, functionName, compileState)

        (vType, vName, vOffset, info) = GetVariableInfo(
            functionName, line.currentValue, compileState
        )
...
def CompileLoopControl(programIR, functionName, line, compileState):
    ...
    compileState.AddStatement(
        "brz {}".format(MakeControlLabel(line.num, line.name, "else"))
    )
...
def CompileLoopModifier(programIR, functionName, line, compileState):
    ...
    if line.name == "Pass":
        ...
        compileState.AddStatement(
            "bra {}".format(MakeControlLabel(line.num, line.name, "else"))
        )
...
def CompileBoolCheck(programIR, functionName, line, compileState):
    ...
    if line.op == "Done":
        ...
        compileState.AddStatement(
            "bra {}".format(MakeControlLabel(line.num, line.op, "end"))
        )
...
def FindFirstAssignmentToGlobal(programIR, name, typeInfo):
            ...
            if typeInfo == "L":
                if line.funcName == "Ed.List1":
                    ...
                    if line.args[1].IsListConst:
                        ...
                        CompileError_NO_RET(
                            23,
                            "FirstAssignment search for type:{} found strange call arg:{}".format(
                                typeInfo, line.args[1].Name
                            ),
                        )
                else:
                    CompileError_NO_RET(
                        24,
                        "FirstAssignment search for type:{} found strange call:{}".format(
                            typeInfo, line.funcName
                        ),
                    )
    ...
    for line in main.body:
        if (
            (line.kind == "UAssign")
            and (line.GetValues())
            and (line.GetTarget() and (line.GetTarget().name == name))
        ):
...
def LayoutClasses(programIR, compileState):
    for func in programIR.Function:
        ...
        if method == "__init__":
            ...
            io.Out.DebugRaw(
                "LayoutClasses - Class {} needs {} words".format(className, words)
            )
...
def LayoutFunctionVars(programIR, compileState):
    for func in programIR.Function:
        ...
        for v in function.localVar:
            if (classData is not None) and (v in classData):
                continue
    ...
    for g in programIR.globalVar:
        ...
        if typeInfo == "I":
            ...
            if g in edpy_values.variables:
                ...
                compileState.AddStatement(
                    "DATW {} {} 1 {:d}".format(
                        g, compileState.wordsUsed, programIR.EdVariables[g]
                    )
                )
            else:
                compileState.AddStatement(
                    "DATW {} {} 1".format(g, compileState.wordsUsed)
                )
            ...
            if init is not None:
                compileState.AddStatement(
                    'DATB {}, {}, {}, "{}"'.format(
                        internalName, start, size, init.strConst
                    )
                )
            else:
                compileState.AddStatement(
                    "DATB {}, {}, {}".format(internalName, start, size)
                )
            ...
            compileState.AddStatement(
                "DATW {}, {}, 1, {}".format(
                    g, compileState.wordsUsed, (start + (size << 8))
                )
            )
            compileState.wordData[g] = (compileState.wordsUsed, 1)
            compileState.wordsUsed += 1
            ...
            if init is not None:
                ...
                if len(values) > 2:
                    values = values[:-2]
                compileState.AddStatement(
                    "DATW {}, {}, {}, {}".format(internalName, start, size, values)
                )

            else:
                compileState.AddStatement(
                    "DATW {}, {}, {}".format(internalName, start, size)
                )
            ...
            start = compileState.wordsUsed
            compileState.AddStatement(
                "DATW {}, {}, {}".format(internalName + "." + className, start, size)
            )
            compileState.wordsUsed += size
            ...
            compileState.AddStatement(
                "DATW {}, {}, 1, {}".format(g, compileState.wordsUsed, start)
            )
            compileState.wordData[g] = (compileState.wordsUsed, 1)
            compileState.wordsUsed += 1
...
def StartEventCall(
    compileState, module, bit, overrideMask=None, overrideValue=None, leaveBitSet=False
):
    mask = 1 << int(bit)
    value = mask
    ...
    compileState.AddStatement(
        "BEGIN EVENT %{}:status, {:d}, {:d}".format(module, mask, value)
    )
    if not leaveBitSet:
...
def AddInEventHandlerWrappers(compileState):
    ...
    for code in compileState.eventHandler:
        ...
        if code == edpy_values.constants["Ed.EVENT_TIMER_FINISHED"]:
            ...
            StartEventCall(
                compileState, "LINE_TRACKER1", 1, overrideMask=3, overrideValue=3
            )

        elif code == edpy_values.constants["Ed.EVENT_LINE_TRACKER_ON_BLACK"]:
            StartEventCall(
                compileState, "LINE_TRACKER1", 1, overrideMask=3, overrideValue=2
            )
...
def Compile(programIR, doOpts):
    ...
    try:
        ...
        io.Out.Error(
            io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", 702
        )
        if io.Out.IsReRaiseSet():
            raise
