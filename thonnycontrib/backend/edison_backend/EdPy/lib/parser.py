...
def CheckCall(node):
    ...
    if (
        (len(node.keywords) == 0)
        and (getattr(node, "starargs", None) is None)  # no keywords
        and (getattr(node, "kwargs", None) is None)  # no starargs Py2
        and  # no kwargs Py2
...
def GetVarName(node, noException=None):
    if Name(node) == "Name":
        return node.id
    elif (Name(node) == "Attribute") and (Name(node.value) == "Name"):
...
def CheckCompare(node):
    ...
    if (len(node.ops) != 1) or (len(node.comparators) != 1):  # no keywords
...
def CheckFor(node):
    ...
    if (
        (Name(node.iter) != "Call")
        or (CheckCall(node.iter) != "range")
        or (len(node.iter.args) != 1)
        or (
            (Name(node.iter.args[0]) != "Num")
            and GetVarName(node.iter.args[0], False) is None
        )
    ):
        io.Out.Error(
            io.TS.PARSE_TOO_COMPLEX,
...
class Converter(object):
    def __init__(self, programIR):
        ...
        self.loopStack = (
            []
        )  # used to associate break/continue with the enclosing while/for
        self.forIndex = (
            10000  # temp var used in for loops. Stay out of normal temp var way
        )
    ...
    def AddEdFunction(self, node):
        ...
        if len(className) > 0:
            if (len(newFunction.args) == 0) or (newFunction.args[0] != "self"):
                io.Out.Error(
                    io.TS.PARSE_CLASS_ARG0_NOT_SELF,
    ...
    def AddFunctionStatement(self, function, node):
        ...
        if name in ("Assign", "AugAssign"):
            ...
            function.body.append(
                program.UAssign(forIndexValue, "UAdd", program.Value(constant=-1))
            )
            ...
            function.body.append(
                program.BAssign(
                    forIndexValue, forIndexValue, "Add", program.Value(constant=1)
                )
            )
            ...
            if forType == "Range":
                ...
                function.body.append(
                    program.ForControl(
                        markerNumber, constantLimit=limit, currentValue=forIndexValue
                    )
                )
                ...
                function.body.append(
                    program.UAssign(
                        program.Value(name=node.target.id), "UAdd", forIndexValue
                    )
                )
                ...
                function.body.append(
                    program.ForControl(
                        markerNumber,
                        arrayValue=program.Value(
                            name=arrayName, iVariable=forIndexTempNumber
                        ),
                    )
                )
                ...
                function.body.append(
                    program.UAssign(
                        program.Value(name=node.target.id),
                        "UAdd",
                        program.Value(name=arrayName, iVariable=forIndexTempNumber),
                    )
                )
    ...
    def AddAssignStatement(self, function, node):
        ...
        if (
            (len(statementList) == rhsStatements)
            or  # Make sure that evaluating target made
            # a statement,
            (statementList[-1].kind != "UAssign")
            or (statementList[-1].operation != "UAdd")  # and it is as expected.
            or (statementList[-1].operand.IsConstant())
        ):
        ...
        if augOp is not None:
            ...
            statementList[-1] = program.BAssign(
                target, target, augOp, program.Value(name=0)
            )
        else:
    ...
    def AddControlStatement(self, function, node, ctlMarker):
        ...
        function.body.append(
            program.LoopControl(ctlMarker, name, program.Value(name=0))
        )
    ...
    def HandleExpr(self, node, statementList, tempCount, lineNo=None):
        ...
        if nodeName == "Num":
            ...
            for v in node.values:
                tempCount += 1
                check = program.BoolCheck(
                    marker,
                    op,
                    program.Value(name=tempCount),
            ...
            statementList.append(
                program.BoolCheck(
                    marker, "Done", resultValue, program.Value(name=resultTemp)
                )
            )
    ...
    def AddClass(self, node):
        ...
        if len(node.bases) > 0:
            io.Out.Error(
                io.TS.PARSE_CLASS_NO_BASES_ALLOWED,
        ...
        for e in node.body:
            ...
            if name == "FunctionDef":
                ...
                io.Out.Error(
                    io.TS.PARSE_CLASS_ALL_STATEMENTS_IN_FUNCTIONS,
    ...
    def AddImport(self, node):
        ...
        if (
            (len(self.program.Function) > 0 and "__main__" not in self.program.Function)
            or (len(self.program.Function) > 1)
            or (len(self.program.Class) > 0)
        ):
            io.Out.Error(
                io.TS.PARSE_IMPORT_ORDER,
...
def ConvertToIR(topNode, programIR, internalAst):
    ...
    try:
        ...
        rtc = 1
        io.Out.Error(
            io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", 701
        )
        if io.Out.IsReRaiseSet():
            raise
...
def Parse(filename, programIR):
    ...
    try:
        ...
        io.Out.Error(
            io.TS.FILE_OPEN_ERROR, "file:0: Could not access file {0}", filename
        )
        if io.Out.IsReRaiseSet():
            raise
...
def NormalPythonParse(programString, filename):
    ...
    try:
        ...
        io.Out.Error(
            io.TS.PARSE_SYNTAX_ERROR, "file:{0}:{1}: Syntax error", s.lineno, s.offset
        )
        if io.Out.IsReRaiseSet():
            raise
        return 2, None
        ...
        io.Out.Error(
            io.TS.BAD_INPUT_CHARS, "file:0: Illegal character in {0}", filename
        )
        if io.Out.IsReRaiseSet():
            raise
        return 2, None
        ...
        io.Out.Error(
            io.TS.PARSE_ERROR, "file:0: There was an error parsing {0}", filename
        )
        if io.Out.IsReRaiseSet():
            raise
        return 2, None
