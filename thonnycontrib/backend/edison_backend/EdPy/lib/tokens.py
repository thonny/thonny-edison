...
class TokenStream(object):
    ...
    def add_token(self, token, explicit_placement=-1):
        ...
        if jump:
            if not jump[1].startswith(":"):
                # a local jump
                token.set_jump_label(
                    jump[0], jump[1] + "_%d" % (self.section_count), jump[2]
                )
        ...
        if explicit_placement < 0:
            ...
            if explicit_placement >= len(self.token_stream):
                AsmError_NO_RET(
                    104, "Explicitly placing a token after the end of the stream."
                )
            else:
    ...
    def add_token_in_fixups(self, token, explicit_placement):
        if explicit_placement >= len(self.token_stream):
            AsmError_NO_RET(
                105, "Explicitly placing a token after the end of the stream."
            )
        else:
            self.token_stream.insert(explicit_placement, token)
    ...
    def set_limits(self, b_lim, w_lim, l_lim, e_handlers, t_lim):
        ...
        for i in range(5):
            if new_limits[i] < 0:
                AsmError_NO_RET(
                    109, "Limit %s can't be less then 0." % (LIMIT_NAMES[i])
                )
            elif new_limits[i] > MAX_LIMITS[i]:
                AsmError_NO_RET(
                    110, "Limit %s too large." % (LIMIT_NAMES[i], MAX_LIMITS[i])
                )
    ...
    def reserve_name_space(self, space, start, length):
        ...
        if (start + length) > self.limits[space]:
            io.Out.ErrorRaw(
                "Reserve space extends beyond the max for %s space."
                % (space_names[space])
            )
            io.Out.Error(
                io.TS.ASM_MEM_OVERFLOW,
                "file::: Overflowed {0} memory",
                space_names[space],
            )
            raise program.AssemblerError
    ...
    def add_variable(self, space, name, start, length=0):
        io.Out.DebugRaw(
            "Add_variable space:%s, name:%s, start:%d, length:%d"
    ...
    def add_begin(self, type, arg1=-1, arg2=-1, arg3=-1):
        ...
        io.Out.DebugRaw(
            "Add_begin type:%s, arg1:%d, arg2:%d, arg3:%d" % (type, arg1, arg2, arg3)
        )
        ...
        if type in ["firmware", "main", "event"]:
            if len(self.current_sections) > 0:
                AsmError_NO_RET(
                    117, "This section %s must be the first section" % (type)
                )

            if self.download_type:
                if self.download_type[0] in ["main", "event"] and type in ["firmware"]:
                    AsmError_NO_RET(
                        118, "Can't mix 'firmware' and 'main'/'event' sections"
                    )
                elif self.download_type[0] == "firmware":
                    AsmError_NO_RET(
                        119,
                        "Can't mix 'firmware' and 'main'/'event' sections "
...
class TokenAnalyser(object):
        ...
        for t in self.token_stream.token_stream:
            for index, space, name in t.var_info:
                if name not in self.name_space_map[space]:
                    AsmError_NO_RET(
                        122,
                        "Variable %s not declared in %s space."
                        % (name, space_names[space]),
                    )
    ...
    def map_variables_in_space(self, space, v_map):
        ...
        io.Out.SetErrorRawContext(
            2, "Mapping variables in %s space" % (space_names[space])
        )
            ...
            for f_start, f_length, f_end in v_free:
                ...
                if start >= f_start and start < f_end:
                    ...
                    if end <= f_end:
                        ...
                        if type == "data":
                            if name in v_map:
                                if v_map[name][0] != start or v_map[name][1] != length:
                                    ...
                                    AsmError_NO_RET(
                                        123,
                                        "Data variable %s declared twice and differently!"
                                        % (name),
                                    )
                            else:
                        ...
                        if type == "data":
                            io.Out.ErrorRaw(
                                "Fixed data variable %s at %d didn't fit!"
                                % (name, start)
                            )
                            io.Out.Error(
                                io.TS.ASM_MEM_OVERFLOW,
                            ...
                            io.Out.ErrorRaw(
                                "No room for Reserved data space at %d" % (start)
                            )
                            io.Out.Error(
                                io.TS.ASM_MEM_OVERFLOW,
                                "file::: Overflowed {0} memory",
        ...
        for name, length in floats:
            ...
            if best_index == -1:
                AsmError_NO_RET(
                    124, "Float data variable %s (len:%d) didn't fit!" % (name, length)
                )
    ...
    def fixup_jumps(self):
        ...
        while 1:
            ...
            while i < len(self.token_stream.token_stream):
                ...
                if t.has_jump_label():
                    index, name, big = t.get_jump_label()
                    my_address = c_lengths[
                        i + 1
                    ]  # The PC is pointing to start of next token
    ...
    def create_header(self):
        ...
        if not self.token_stream.version:
            ...
            if self.token_stream.download_type[0] == "firmware":
                if (self.token_stream.version[0] != 0xE) and (
                    ...
                    AsmError_NO_RET(
                        128,
                        "This assembler only handles firmware "
                ...
                if self.token_stream.version[0] != 0x6:
                    AsmError_NO_RET(
                        129,
                        "This assembler only handles token "
                        + "major versions 0x6 (not 0x%x)"
                        % (self.token_stream.version[0]),
                    )
        ...
        if self.token_stream.download_type[0] == "firmware":
            ...
            if is_bad_length_for_crc(len(bytes)):
                print(
                    "Warning - skipping bad CRC length at {} bytes.".format(len(bytes))
                )
                added_bytes = 1
                bytes.extend([0xFF])
            ...
            # fixup main offset
            header_list[6], header_list[7] = word_to_bytes(
                main_offset + final_header_bytes
            )
            ...
            if is_bad_length_for_crc(len(new_bytes)):
                print(
                    "Warning - skipping bad CRC length at {} bytes.".format(
                        len(new_bytes)
                    )
                )
                added_bytes += 1
...
class Token(object):
    ...
    def print_token(self):
        ...
        if self.binary_file:
            print(
                "Inserted %d binary bytes from %s:\n   [" % (length, self.binary_file),
                end="",
            )
        ...
        if self.binary_file:
            ...
            print(
                "(jump_ref to %s, big? %d)" % (self.jump_label[1], self.jump_label[2])
            )
