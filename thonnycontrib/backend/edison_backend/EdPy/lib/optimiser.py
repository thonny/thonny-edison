...
def IsSimpleTemp(value):
    return (
        ...
        and value.IsTemp()
        and (value.name < value.loopTempStart)
    )
...
def VerifyTargetIsTuneStringElement(
    value, localVar, globalVar, globalWriteAccess, line
):
    if not value.IsAssignable():
        io.Out.Error(
            io.TS.OPT_NOT_ASSIGNABLE,
            "file:{0}:: Syntax Error, no assignable variable",
            line,
        )
        raise program.OptError
...
def CheckWriteVarTypeAddIfMissing(
    value, typeInfo, localVar, globalVar, globalWriteAccess, inFunc, line
):
    ...
    if existingTypeInfo:
        if (
            (typeInfo == existingTypeInfo)
            or ((existingTypeInfo[0] == "T") and (typeInfo == ("S", 1)))
            or ((existingTypeInfo[0] == "L") and (typeInfo[0] != "L"))
        ):
            pass
        else:
            io.Out.Error(
...
def VerifySignature(newArgList, oldArgList, callName, line):
    ...
    for i in range(len(newArgList)):
        ...
        if len(newArg[0]) != 1:
            io.Out.FatalRaw(
                "New argument should only have 1 type. It was {}.".format(newArg)
            )
    ...
    if value:
        if value.IsSliceWithSimpleTempIndex():
            ...
            if v is not None:
                if v.IsIntConst():
                    io.Out.DebugRaw(
                        "{0}-{1}: replacing slice temp index {2} with constant {3}".format(
                            opStr, line, value.name, v.constant
                        )
                    )
                    return program.Value(name=value.name, iConstant=v.constant)
                if v.IsSimpleVar():
                    io.Out.DebugRaw(
                        "{0}-{1}: replacing slice temp index {2} with variable {3}".format(
                            opStr, line, value.name, v.name
                        )
                    )
        ...
        # Separated in case want to disable one of these
            ...
            if v is not None:
                if v.IsIntConst():
                    io.Out.DebugRaw(
                        "{0}-{1}: replacing slice index {2} with constant {3}".format(
                            opStr, line, value.name, v.constant
                        )
                    )
                    return program.Value(name=value.name, iConstant=v.constant)
                if v.IsSimpleVar():
                    io.Out.DebugRaw(
                        "{0}-{1}: replacing slice index {2} with variable {3}".format(
                            opStr, line, value.name, v.name
                        )
                    )
                    return program.Value(name=value.name, iVariable=v.name)
            ...
            if op.kind == "Marker":
                ...
                if op.left.IsConstant() and op.right.IsConstant():
                    ...
                    if not IsSimpleTemp(target):
                        ...
                        newBody.append(
                            program.UAssign(target=op.target, op="UAdd", operand=value)
                        )
                    else:
                        io.Out.DebugRaw("Removed assignment in line:", line, op)
                ...
                for i in range(len(op.args)):
                    ...
                    newValue = CheckAndReplaceListIndex(
                        op.args[i], rewriteList, line, "CR"
                    )
                    if newValue:
                        op.args[i] = newValue
                        argChange = True
                ...
                # handle list indicies
                newValue = CheckAndReplaceListIndex(
                    op.returnValue, rewriteList, line, "CR"
                )
                if newValue:
        ...
        for i in range(len(body)):
            ...
            if op.kind == "Marker":
                ...
                if newValue:
                    op.target = newValue
                newValue = CheckAndReplaceListIndex(
                    op.operand, rewriteList, line, "SVR"
                )
                if newValue:
                    op.operand = newValue
                ...
                if not op.operand.IsConstant():
                    ...
                    if (
                        (op.operation == "UAdd")
                        and (IsSimpleTemp(target) or (controlLevel == 0))
                        and (IsSimpleTemp(target) or not IsSimpleTemp(op.operand))
                    ):
                ...
                for i in range(len(op.args)):
                    ...
                    newValue = CheckAndReplaceListIndex(
                        op.args[i], rewriteList, line, "SVR"
                    )
                    if newValue:
                        op.args[i] = newValue
                        argChange = True
                ...
                # handle list indicies
                newValue = CheckAndReplaceListIndex(
                    op.returnValue, rewriteList, line, "SVR"
                )
                if newValue:
                    op.returnValue = newValue
                    argChange = True
        ...
        for i in range(len(body)):
            ...
            if op.kind == "Marker":
                ...
                if op.target and IsSimpleTemp(op.target):
                    if (
                        (i < (len(body) - 1))
                        and (body[i + 1].kind == "UAssign")
                        and (body[i + 1].operation == "UAdd")
                        and (body[i + 1].operand == op.target)
                    ):
                        ...
                        msg = (
                            "SCC-{0} Collapse Call {1} to skip temp assignment".format(
                                line, op.funcName
                            )
                        )
        ...
        for v in op.GetValues():
            if v.IsSimpleTemp():
                if v.name not in newTempNames:
                    ...
                    if v.name != tempCount:
                        io.Out.DebugRaw(
                            "TEC-{0} renaming simple temp value:{1} to {2}".format(
                                line, v.name, newTempNames[v.name]
                            )
                        )
                    tempCount += 1
                ...
                if v.indexVariable not in newTempNames:
                    ...
                    if v.name != tempCount:
                        io.Out.DebugRaw(
                            "TEC-{0} renaming simple temp index:{1} to {2}".format(
                                line, v.indexVariable, newTempNames[v.name]
                            )
                        )
                    tempCount += 1
        ...
        if t is not None:
            if t.IsSimpleTemp():
                if t.name not in newTempNames:
                    ...
                    if t.name != tempCount:
                        io.Out.DebugRaw(
                            "TEC-{0} renaming simple temp target:{1} to {2}".format(
                                line, t.name, newTempNames[t.name]
                            )
                        )
                    tempCount += 1
                ...
                if t.indexVariable not in newTempNames:
                    ...
                    if t.name != tempCount:
                        io.Out.DebugRaw(
                            "TEC-{0} renaming simple temp target:{1} to {2}".format(
                                line, t.indexVariable, newTempNames[t.name]
                            )
                        )
                    tempCount += 1
    ...
    if tempCount == 0:
        ...
        for op in lineBuffer:
            if op.kind == "LoopControl":
                newOp = program.LoopControl(
                    op.num, op.name, NewSimpleTemp(op.test, newTempNames, line)
                )

            elif op.kind == "ForControl":
                newOp = program.ForControl(
                    op.num,
                    NewSimpleTemp(op.arrayValue, newTempNames, line),
                    NewSimpleTemp(op.constantLimit, newTempNames, line),
                    NewSimpleTemp(op.currentValue, newTempNames, line),
                )

            elif op.kind == "BoolCheck":
                newOp = program.BoolCheck(
                    op.num,
                    op.op,
                    NewSimpleTemp(op.value, newTempNames, line),
                    NewSimpleTemp(op.target, newTempNames, line),
                )

            elif op.kind == "UAssign":
                newOp = program.UAssign(
                    NewSimpleTemp(op.target, newTempNames, line),
                    op.operation,
                ...
                newOp = program.Call(
                    NewSimpleTemp(op.target, newTempNames, line), op.funcName, args
                )

            elif (op.kind == "Return") and (not op.IsVoidReturn()):
                newOp = program.Return(
                    NewSimpleTemp(op.returnValue, newTempNames, line)
                )
        ...
        if function.maxSimpleTemps > 0:
            ...
            for i in range(len(body)):
                ...
                if op.kind == "Marker":
                    ...
                    if len(lineBuffer) > 0:
                        lineBuffer, maxTemps = ReduceSimpleTempsInOneLine(
                            lineBuffer, lastLine
                        )
...
def QueryEdPyConstantUse(value, constants, line):
    ...
    if (
        value
        and value.IsSimpleVar()
        and (not value.IsTemp())
        and (value.name == "None")
    ):
        io.Out.Error(
            io.TS.OPT_NOT_SUPPORTED,
    ...
    if (
        ...
        and value.IsSimpleVar()
        and (not value.IsTemp())
        and (value.name in constants)
    ):
        io.Out.DebugRaw(
            "EPC-{0} replacing var {1} with constant {2}".format(
                line, value.name, constants[value.name]
            )
        )
    ...
    for f in programIR.Function:
        ...
        for op in body:
            ...
            if target and target.IsSimpleVar() and (target.name in constants):
                io.Out.Error(
                    io.TS.OPT_WRITE_TO_ED_PY_CONSTANT,
    ...
    while len(callList) > oldCallListLen:
        ...
        for funcName in oldCallList:
            ...
            if funcName in programIR.Function:
                ...
                io.Out.Error(
                    io.TS.OPT_FUNCTION_NOT_DEFINED,
...
def RemoveUselessMarkers(programIR):
    ...
    for f in programIR.Function:
        ...
        for i in range(opCount):
            ...
            if i < (opCount - 1):
                if (body[i].kind == "Marker") and (body[i + 1].kind == "Marker"):
                    skip = True
                    io.Out.DebugRaw(
                        "RUM-{0} remove unused line marker".format(body[i].line)
                    )
            ...
            if op.kind == "Marker":
                ...
                for v in values:
                    if v.IsIntConst():
                        ...
                        if v.constant < -32767:
                            io.Out.Error(
                                io.TS.OPT_CONSTANT_TOO_NEGATIVE,
                            ...
                            io.Out.Error(
                                io.TS.OPT_CONSTANT_TOO_POSITIVE,
                                "file:{0}:: Syntax Error, constant {1} is out of range",
                                line,
                                v.constant,
                            )
                            ...
                            io.Out.DebugRaw(
                                "VCR-{0} constant {1} is ok, value {2}".format(
...
def VerifyEdisonVariables(programIR):
            ...
            if t is not None:
                if t.Name() in varNames:
                    ...
                    if (op.kind != "UAssign") or (op.operand.IsIntConst() == False):
                        # Not a constant
                        io.Out.Error(
                            io.TS.OPT_ED_ASSIGN_NOT_CONSTANT,
    ...
    if (
        version == edpy_values.constants["Ed.V1"]
        and distance != edpy_values.constants["Ed.TIME"]
    ):
        io.Out.Error(
            io.TS.OPT_ED_FUNCTION_NOT_AVAILABLE,
            "file:{0}:: Syntax Error, {1} is not available in Edison Version {2}",
            varLines["Ed.DistanceUnits"],
            "drive by distance",
            version,
        )
        raise program.OptError
        ...
        for op in body:
            if op.kind == "Marker":
                ...
                if (t is not None) and (t.Name() in varNames):
                    io.Out.Error(
                        io.TS.OPT_ED_ASSIGN_IN_FUNCTION,
        ...
        for op in body:
            if op.kind == "Marker":
                ...
                if op.kind == "Call":
                    ...
                    if (
                        varValues["Ed.DistanceUnits"]
                        == edpy_values.constants["Ed.TIME"]
                    ) and (op.funcName in notUsedWithTime):
                        io.Out.Error(
                            io.TS.OPT_ED_FUNCTION_NOT_USEFUL,
                    ...
                    if op.funcName in possibleInlineFunctions:
                        ...
                        for i in range(len(op.args)):
                            ...
                            if i == 2:
                                ...
                                if (
                                    op.args[i].constant
                                    == edpy_values.constants["Ed.DISTANCE_UNLIMITED"]
                                ) and (len(op.args) == 3):
                                    changeToInline = True
                                # check if this is a STOP -- distance is not important then
                                elif (
                                    op.args[0].constant
                                    == edpy_values.constants["Ed.STOP"]
                                ) and (len(op.args) == 3):
                                    changeToInline = True
                        ...
                        if changeToInline:
                            ...
                            io.Out.DebugRaw(
                                "VEV-{0} Added suffix to function - now: {1}".format(
                                    line, op.funcName
                                )
                            )
        ...
        if op.kind == "Marker":
            ...
            if op.funcName == "Ed.List":
                ...
                if (op.target is None) or (not op.target.IsSimpleVar()):
                    io.Out.Error(
                        io.TS.PARSE_SYNTAX_ERROR, "file:{0}:{1}: Syntax error", line, ""
                    )
                ...
                if len(op.args) == 1:
                    ...
                    if len(op.args[1].strConst) > op.args[0].constant:
                        ...
                        len(op.args[1].strConst) > 0 and op.args[1].strConst[-1] != "z"
                    ):
                        io.Out.Warning(
                            io.TS.OPT_ED_WARN_TUNESTRING_END,
...
def VerifyValidSliceTarget(target, funcName, glbVar, lclVar, line):
    ...
    if target.IsSlice():
        ...
        if (n in lclVar) and (lclVar[n][0] not in ("T", "L")):
            io.Out.Error(
                io.TS.OPT_VAR_MUST_BE_TS_OR_LIST,
...
def TypeVariablesByFunc(programIR, funcName, callList):
    ...
    if funcName not in programIR.Function:
        io.Out.DebugRaw(
            "Function {0} must be external -- later though will have to be supplied!".format(
                funcName
            )
        )
        io.Out.Error(
            io.TS.OPT_UNKNOWN_FUNCTION,
    ...
    if len(function.args) != len(sigDict[funcName]):
        io.Out.Error(
            io.TS.OPT_INCORRECT_ARG_DEFINE,
        ...
        if op.kind == "Marker":
            ...
            if op.operand.IsStrConst():
                ...
                if (len(op.operand.strConst) == 1) and target.IsSlice():
                    ...
                    VerifyTargetIsTuneStringElement(
                        target, lclVar, glbVar, glbAccess, line
                    )
                ...
                if op.operand.IsTSRef():
                    CheckWriteVarTypeAddIfMissing(
                        target, ("T", None), lclVar, glbVar, glbAccess, inFunc, line
                    )

                elif op.operand.IsListRef():
                    CheckWriteVarTypeAddIfMissing(
                        target, ("L", None), lclVar, glbVar, glbAccess, inFunc, line
                    )

                elif op.operand.IsObjRef():
                    # TODO: Need class name
                    CheckWriteVarTypeAddIfMissing(
                        target, ("O", None), lclVar, glbVar, glbAccess, inFunc, line
                    )

                elif op.operand.IsSimpleVar():
                    scope, typeInfo = GetReadVarType(
                        op.operand.name, lclVar, glbVar, line
                    )
                    # Just copy it through (so allow none integers)
                    CheckWriteVarTypeAddIfMissing(
                        target, typeInfo, lclVar, glbVar, glbAccess, inFunc, line
                    )

                else:
                    CheckWriteVarTypeAddIfMissing(
                        target, ("I", None), lclVar, glbVar, glbAccess, inFunc, line
                    )
            ...
            if op.IsArray():
                ...
                scope, typeInfo = GetReadVarType(
                    op.arrayValue.name, lclVar, glbVar, line
                )
            ...
            if callName == "Ed.RegisterEventHandler":
                ...
                if (
                    not op.args[0].IsIntConst()
                    or (op.args[0].constant < 0)
                    or (
                        op.args[0].constant
                        > edpy_values.constants["Ed.EVENT_LAST_EVENT"]
                    )
                ):
                    io.Out.Error(
                        io.TS.OPT_BAD_EVENT_NUMBER,
                ...
                if var in programIR.Class:
                    if method == "__init__":
                        if (
                            ...
                            or (op.target.IsTemp())
                            or (not op.target.IsSimpleVar())
                        ):
                            io.Out.Error(
                                io.TS.OPT_ONLY_AT_TOP_LEVEL,
                    ...
                    CheckWriteVarTypeAddIfMissing(
                        op.target, ("S", 1), lclVar, glbVar, glbAccess, inFunc, line
                    )

                else:
                    CheckWriteVarTypeAddIfMissing(
                        op.target, ("I", None), lclVar, glbVar, glbAccess, inFunc, line
                    )
...
def Optimise(programIR):
    ...
    try:
        ...
        rtc = 1
        io.Out.Error(
            io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", 700
        )
        if io.Out.IsReRaiseSet():
            raise
    ...
    if io.Out.GetInfoDumpMask() & io.DUMP.OPTIMISER:
        io.Out.DebugRaw(
            "\nDump of internal representation after OPTIMISATION (rtc:{0}):".format(
                rtc
            )
        )
        programIR.Dump()
