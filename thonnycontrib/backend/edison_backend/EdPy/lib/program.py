...
class ControlMarker(object):
    ...
    def __repr__(self):
        msg = "<program.ControlMarker marker:{0} {1} {2}>".format(
            self.num, self.name, self.end
        )
        return msg
...
class LoopModifier(object):
    ...
    def __repr__(self):
        msg = "<program.LoopModifier {0}, name:{1}>".format(self.num, self.name)
        return msg
...
class ForControl(object):
    ...
    def __init__(
        self, markerNumber, arrayValue=None, constantLimit=None, currentValue=None
    ):
        ...
        if (
            (self.arrayValue is None and self.constantLimit is None)
            or (self.arrayValue is not None and self.constantLimit is not None)
            or (self.currentValue is None and self.constantLimit is not None)
            or (self.currentValue is not None and self.constantLimit is None)
        ):
    ...
    def __repr__(self):
        ...
        if self.IsArray():
            ...
            msg += "constantLimit:{0}, currentValue:{1}>".format(
                self.constantLimit, self.currentValue
            )
...
class Value(object):
    ...
    def __init__(
        ...
        self.indexConstant = (
            iConstant  # if not None, then the value is a slice at this index
        )
        ...
        self.objectRef = (
            objectRef  # if not None, then a reference to an object variable
        )
        ...
        if (
            ...
            or ((self.indexConstant is not None) and (self.indexVariable is not None))
            or ((self.indexConstant is not None) and (self.name is None))
            or ((self.indexVariable is not None) and (self.name is None))
            or (
                (self.tsRef is not None)
                and ((self.listRef is not None) or (self.objectRef is not None))
            )
            or (
                (self.listRef is not None)
                and ((self.tsRef is not None) or (self.objectRef is not None))
            )
            or (
                (self.objectRef is not None)
                and ((self.listRef is not None) or (self.tsRef is not None))
            )
        ):
    ...
    def IsSliceWithSimpleTempIndex(self):
        return (
            self.IsSlice()
            and self.indexVariable is not None
            and type(self.indexVariable) is int
            and (self.indexVariable < self.loopTempStart)
        )
    ...
    def IsSliceWithVarIndex(self):
        return (
            self.IsSlice()
            and self.indexVariable is not None
            and type(self.indexVariable) is not int
        )
    ...
    def __eq__(self, rhs):
        return (
            ...
            and (self.indexConstant == rhs.indexConstant)
            and (self.indexVariable == rhs.indexVariable)
            and (self.constant == rhs.constant)
            and (self.strConst == rhs.strConst)
            and (self.listConst == rhs.listConst)
            and (self.tsRef == rhs.tsRef)
            and (self.listRef == rhs.listRef)
            and (self.objectRef == rhs.objectRef)
        )
...
class UAssign(object):
    ...
    def __init__(self, target=None, op=None, operand=None):
        ...
        self.operand = (
            operand  # (used for binary op or unary op) if used then a Value object
        )
