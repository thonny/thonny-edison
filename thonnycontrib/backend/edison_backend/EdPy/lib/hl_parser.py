...
def add_device(loc, dtype, name=None):
    ...
    if loc_hex in locations:
        AsmError_NO_RET(
            202,
            "Location (%d) is already used with a unit of type: " % (loc)
            + locations[loc_hex][0],
        )
        return False
    ...
    if name in special_names:
        AsmError_NO_RET(
            203,
            "DEVICE name can not be one of the built-in names " + str(special_names),
        )
        return False
    ...
    if dtype == "tracker" and loc != 0:
        AsmError_NO_RET(
            205, "A tracker device can only be located at 0, not location %d" % (loc)
        )
        return False
    ...
    if dtype == "motor-a" or dtype == "motor-b":
        ...
        if motor_second_hex in locations:
            AsmError_NO_RET(
                206,
                "Motors require two locations yet the second location is already used with a "
...
def parse_mod_reg(smodreg):
    ...
    if (
        (len(smodreg) == 2)
        and (smodreg[0] in string.hexdigits)
        and (smodreg[1] in string.hexdigits)
    ):
        num = int(smodreg, 16)
    else:
        if ":" in smodreg:
            ...
            if part[0] in devices:
                ...
                AsmError_NO_RET(
                    213, "Didn't understand the module name/number: " + part[0]
                )
                return 0
            ...
            if part[1] in regs:
                ...
                AsmError_NO_RET(
                    214, "Didn't understand the register name/number: " + part[1]
                )
                return 0
