...
def AsmError_NO_RET(number, internalError=None, line=0):
    ...
    io.Out.Error(
        io.TS.ASM_INTERNAL_ERROR, "file::: Assembler internal error {0}", number
    )
...
def assem_file(f_name, called_from=[], pass_err=None):
    ...
    for line in fh:
        ...
        if (
            (len(insert_check) >= 3)
            and (insert_check[0] == "INSERT")
            and (insert_check[1].lower() == "tokens")
        ):
...
def assem_move(size, words, special, line):
    io.Out.DebugRaw(
        "Move size:%d, words:%s" % (size, hl_parser.format_word_list(words))
    )
    if len(words) != 2:
...
def assem_move_from_acc(size, words, special, line):
    ...
    if special == "mol":
        ...
        elif (words[0].type() != "var") or (words[1].type() != "modreg"):
...
def assem_move_not_from_acc(size, words, special, line):
    ...
    if words[1].type() == "modreg":
        ...
        if size == 0:
            ...
            if special:
                AsmError_NO_RET(
                    10, "Desination for lcd and time moves can not be in word variables"
                )
...
def assem_data(size, words, line):
    ...
    if length > 0 and len(values) > length:
        AsmError_NO_RET(
            14,
            "Data has length of %d but more values (%d) then length"
            % (length, len(values)),
        )
...
def assem_uni_math(op, size, words, line):
    io.Out.DebugRaw(
        "Mathu %s size:%d, words:%s" % (op, size, hl_parser.format_word_list(words))
    )
    if len(words) > 1:
    ...
    if not words or (words[0].type() == "modreg" and words[0].val() == 0xF0):
...
def assem_basic_math(op, size, words, line):
    io.Out.DebugRaw(
        "Mathb %s size:%s, words:%s" % (op, size, hl_parser.format_word_list(words))
    )
    if len(words) != 1:
...
def assem_other_math(op, size, words, line):
    io.Out.DebugRaw(
        "Matho %s size:%d, words:%s" % (op, size, hl_parser.format_word_list(words))
    )
...
def assem_stack(op, size, words, line):
    io.Out.DebugRaw(
        "Stack %s size:%d, words:%s" % (op, size, hl_parser.format_word_list(words))
    )
...
def assem_debug_output(op, size, words, line):
    io.Out.DebugRaw(
        "Debug output %s size:%d, words:%s"
        % (op, size, hl_parser.format_word_list(words))
    )
    ...
    if words[0].type() != "var":
        AsmError_NO_RET(
            31,
            "Error - Debug output only handles variables - invalid operand: %s"
            % (words[0].type()),
        )
    ...
    if op == "push":
        if words[0].type() == "modreg":
            if words[0].val() == 0xF0:  # ACC
                ...
                AsmError_NO_RET(
                    33, "Push with no size - invalid operand: %s" % (words[0].type())
                )
        else:
            AsmError_NO_RET(
                34, "Push with no size - invalid operand: %s" % (words[0].type())
            )
...
def assem_jump(op, cond, words, line):
    io.Out.DebugRaw(
        "Jump %s cond:%s, words:%s" % (op, cond, hl_parser.format_word_list(words))
    )
...
def assem_misc(op, words, line):
    ...
    if op == "stop":
        ...
        if bit < 0 or bit > 7:
            AsmError_NO_RET(
                45, "Bitset/bitclr bit must be between 0 and 7 (not %d)" % (bit)
            )
...
def assem_spec_data(which, words, line):
    io.Out.DebugRaw(
        "Spec_data which:%s words:%s" % (which, hl_parser.format_word_list(words))
    )
    ...
    if test <= 0:
        ...
        io.Out.Error(io.TS.ASM_MEM_OVERFLOW, "file::: Overflowed {0} memory", which)
        raise program.AssemblerError
...
def assem_spec_data_lcd(which, words, line):
    io.Out.DebugRaw(
        "Spec_data_lcd which:%s words:%s" % (which, hl_parser.format_word_list(words))
    )
...
def assem_spec_binary(which, words, line):
    io.Out.DebugRaw(
        "Spec_binary which:%s words:%s" % (which, hl_parser.format_word_list(words))
    )
...
def assem_spec_reserve(which, words, line):
    io.Out.DebugRaw(
        "Spec_reserve which:%s words:%s" % (which, hl_parser.format_word_list(words))
    )
    if len(words) != 2:
        AsmError_NO_RET(59, "RESERV[ABW] needs 2 arguments: start, length")

    token_stream.reserve_name_space(
        tokens.space_types[which], words[0].anum(), words[1].anum()
    )
...
def assem_spec_begin_end(op, words, line):
    io.Out.DebugRaw(
        "Spec_begin_end op:%s words:%s" % (op, hl_parser.format_word_list(words))
    )
    if len(words) < 1:
...
def assem_spec_device(words, line):
    ...
    if hl_parser.add_device(location, device_type, name):
        token_stream.add_device(
            hl_parser.device_types[device_type],
            location,
...
def assemble_file(srcPath, debug):
    ...
    try:
        ...
        ok = False
        io.Out.Error(
            io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", 703
        )
        if io.Out.IsReRaiseSet():
            raise
...
def assemble_lines(lines, debug):
    ...
    try:
        ...
        ok = False
        io.Out.Error(
            io.TS.CMP_INTERNAL_ERROR, "file::: Compiler internal error {0}", 704
        )
        if io.Out.IsReRaiseSet():
            raise
...
def test():
    ...
    test_simp = [
        ...
        "subw $-3000",
        "movb $0, @big_count",
        "movw @last_reading, @store_reading",
    ]
    ...
    test_device = [
        ...
        "DEVICE motor 1 **",
        "DEVICE digin, 1",
        "DEVICE motor 2, left_wheel",
        "DEVICE motor 4 right_wheel",
        "DEVICE digout, 5",
        "DEVICE analogin 6",
    ]
    test_begin_end = [
        ...
        "BEGIN PROGRAM 1",
        "BEGIN CONFIG",
        "BEGIN EVENT 34, 43,44",
        "END PROGRAM CONFIG 0 0",
        "BEGIN CONFIG 0,0",
        "BEGIN PROGRAM",
        "BEGIN EVENT %acc 0xfe 20/16",
    ]
    ...
    test_prog1 = [
        ...
        "DATB buffer *, 10",
        "DATW temps *, 5",
        'DATA message * * "This is fun but long - will it work?"',
        "movb 5, @count",
        ":b1",
        "movb @count, %acc",
        "cmpb 0",
